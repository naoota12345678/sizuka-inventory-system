...
class GoogleSheetsSync:
    ...
    def authenticate(self):
        """Google Sheets APIの認証（Cloud Run対応版）"""
        try:
            # Cloud Run環境の検出
            if os.getenv('K_SERVICE'):
                logger.info("Cloud Run環境を検出。デフォルト認証を使用します")
                ...
                if self.credentials_file and os.path.exists(self.credentials_file):
                    ...
                    # APIキーを試みる（読み取り専用の公開スプレッドシート用）
                    api_key = os.getenv('GOOGLE_API_KEY')
                    if api_key:
                        logger.info("APIキーを使用")
                        self.service = build('sheets', 'v4', developerKey=api_key)
                        return
                    else:
                        raise ValueError(
                            "認証情報が見つかりません。以下のいずれかを設定してください:\n"
                            "1. GOOGLE_CREDENTIALS_FILE (JSONファイルのパス)\n"
                            "2. GOOGLE_SERVICE_ACCOUNT_JSON (JSON文字列)\n"
                            "3. GOOGLE_API_KEY (公開スプレッドシート用)\n"
                            "4. Cloud Run環境でデフォルト認証"
                        )
    ...
    def read_sheet(self, sheet_name: str, range_name: str = 'A:Z') -> List[List[str]]:
        ...
        try:
            ...
            if error.resp.status == 403:
                logger.error(
                    "アクセス権限エラー。以下を確認してください:\n"
                    f"1. スプレッドシート ID: {self.spreadsheet_id}\n"
                    "2. サービスアカウントにスプレッドシートの閲覧権限があるか\n"
...
if __name__ == "__main__":
    ...
    print("\n認証方法（いずれか1つ）:")
    print("1. Cloud Run環境: 自動的にデフォルト認証を使用")
    print("2. GOOGLE_CREDENTIALS_FILE: Google認証情報ファイルのパス")
    print("3. GOOGLE_SERVICE_ACCOUNT_JSON: サービスアカウントのJSON文字列")
    print("4. GOOGLE_API_KEY: APIキー（公開スプレッドシートのみ）")
