...
# -*- coding: utf-8 -*-
"""
正しいSKUマッピングを使った在庫減少処理
rakuten_item_number（4桁SKU）を使用
"""
    ...
    while True:
        try:
            ...
            result = supabase.table('order_items').select(
                'quantity, product_code, choice_code, product_name, rakuten_item_number'
...
def correct_inventory_reduction():
    """
    正しいSKUマッピング在庫減少処理
    rakuten_item_number（4桁SKU）を使用
    """
    print("=" * 60)
    print("正しいSKUマッピング在庫減少処理開始")
    print("=" * 60)
    
    try:
        # Step 1: 正しいマッピングテーブル取得
        print("Step 1: 正しいマッピングデータ取得中...")
        ...
        for item in pm_data.data:
            ...
            if sku and len(str(sku)) == 4:  # 4桁のSKUのみ
        ...
        print("\n4桁SKUマッピングサンプル:")
        sample_count = 0
        for sku, common_code in sku_mapping.items():
            if sample_count < 5:
                print(f"  - SKU {sku} -> {common_code}")
                sample_count += 1
        ...
        # Step 3: 正しいマッピングによる在庫減少量計算
        print("\nStep 3: 正しいSKUマッピングで在庫減少量計算中...")
        ...
        for item in all_sales_data:
            ...
            # 正しいフィールドを使用
            rakuten_item_number = item.get('rakuten_item_number', '')  # 4桁SKU
            choice_code = item.get('choice_code', '')
            product_code = item.get('product_code', '')  # 8桁管理番号（参考用）
        ...
        # マッピング成功率計算
        success_rate = mapped_count / len(all_sales_data) * 100 if all_sales_data else 0
...
if __name__ == "__main__":
    try:
        success, results = correct_inventory_reduction()
