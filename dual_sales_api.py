...
def get_basic_sales_summary(start_date: str = None, end_date: str = None):
    """
    A) 基本売上集計 - 共通コード単位（在庫管理と同じ仕組み）
    choice_code → common_code → 売上集計
    """
    ...
    print(f"=== A) 基本売上集計 ({start_date} ~ {end_date}) ===")
    ...
    common_code_sales = defaultdict(lambda: {
        ...
        'total_amount': 0,
        'orders_count': 0,
        'choice_codes': set()  # どの選択肢コードからの売上か
    })
    ...
    for i, product in enumerate(sales_list[:5], 1):
        choice_str = ', '.join(product['choice_codes'][:3])  # 最初の3つのchoice_code
...
def get_choice_detail_analysis(start_date: str = None, end_date: str = None):
    """
    B) 選択肢詳細分析 - choice_code単位の人気分析
    R05、R13等の個別商品の人気度を分析
    """
    supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
...
def get_comprehensive_sales_analysis(start_date: str = None, end_date: str = None):
    """
    包括的売上分析 - A) + B) の両方を含む
    """
    print("=== 包括的売上分析 ===")
...
if __name__ == "__main__":
    # 包括的分析の実行
    result = get_comprehensive_sales_analysis('2025-07-31', '2025-08-04')
    
    print(f"\\n=== 分析結果サマリー ===")
    print(f"基本売上: {result['basic_sales']['summary']['total_sales']:,.0f}円")
    print(f"選択肢売上: {result['choice_analysis']['summary']['total_sales']:,.0f}円")
    print(f"マッピング成功率: {result['basic_sales']['summary']['mapping_success_rate']:.1f}%")