@echo off
echo === Alternative Google Sheets Setup ===
echo.

echo 1. Adding credentials file to .dockerignore exceptions...
echo !google-credentials.json >> .dockerignore

echo.
echo 2. Updating Dockerfile to include credentials...
echo # Add after COPY . . line in Dockerfile:
echo # COPY google-credentials.json /app/credentials.json
echo # ENV GOOGLE_APPLICATION_CREDENTIALS=/app/credentials.json

echo.
echo 3. Deploying with credentials included in image...
