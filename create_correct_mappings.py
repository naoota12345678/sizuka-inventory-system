...
# -*- coding: utf-8 -*-
"""
正しいテーブル構造に合わせた選択肢コードマッピングデータを作成
choice_info (jsonb) カラムを使用
"""
...
print("=== Creating Choice Code Mappings (Correct Structure) ===")
...
try:
    ...
    for mapping in sample_mappings:
        try:
            # choice_info (jsonb) フィールドにchoice_codeを格納
            choice_info = {
            ...
            # データの準備
            mapping_data = {
                "choice_info": choice_info,  # jsonb型
                "common_code": mapping["common_code"],
                "product_name": mapping["product_name"],
                "rakuten_sku": mapping["choice_code"],  # choice_codeをrakuten_skuとしても保存
                "created_at": datetime.now(timezone.utc).isoformat()
            }
            
            # 重複チェック（common_codeで）
            existing = supabase.table("choice_code_mapping").select("id").eq("common_code", mapping["common_code"]).execute()
            
            if existing.data:
                # 更新
                result = supabase.table("choice_code_mapping").update(mapping_data).eq("common_code", mapping["common_code"]).execute()
                action = "UPDATED"
            else:
                # 新規挿入
            ...
            if result.data:
                print(f"   OK {action}: {mapping['choice_code']} -> {mapping['common_code']} ({mapping['product_name']})")
                success_count += 1
            else:
    ...
    if total.data:
        print(f"\nSample data:")
        for item in total.data[:3]:
            choice_info = item.get('choice_info', {})
            choice_code = choice_info.get('choice_code', 'Unknown')
            print(f"  {choice_code} -> {item.get('common_code')} ({item.get('product_name', '')[:30]}...)")
